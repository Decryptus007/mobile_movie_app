    ...
    <View className="bg-primary flex-1">
      <ScrollView contentContainerStyle={{ paddingBottom: 80 }}>
        ...
        <View className="flex-col items-start justify-center mt-5 px-5">
          ...
          <View className="flex-row items-center mt-2 gap-x-1 bg-dark-100 px-2 py-1 rounded-md">
            <Image source={icons.star} className="size-4" />
            <Text className="text-white font-bold text-sm">
              {Math.round(movie?.vote_average ?? 0)} / 10
            </Text>
            <Text className="text-light-200 ml-4 text-sm">
          ...
          <View className="flex-row flex justify-between w-1/2">
            <MovieInfo
              label="Budget"
              value={`$${Number(movie?.budget / 1_000_000).toFixed(2)} million`}
            />
            <MovieInfo
              label="Revenue"
              value={`$${Number(movie?.revenue / 1_000_000).toFixed(
                2
              )} million`}
            />
          </View>
